...
function sent04BattleReport() {
  var spreadsheetId    = '1JrR6saVcWD6K0h6J67cgfHz6VDcDRRDMIHseu_f_Nzs';
  var sheetName        = '04戰報製作區';
  var controlSheetName = '04戰報日期區間';
  ...
  var subjectdate   = formatDate_(controlSheet.getRange('C2').getValue());
  var dateRangeText = formatDateTime_(controlSheet.getRange('L2').getValue());
  var sheetTabName  = formatDateCompact_(controlSheet.getRange('C2').getValue());

  var subjectnote = controlSheet.getRange('N2').getValue().toString();
  var bodynote    = controlSheet.getRange('N3').getValue().toString();

  var subject  = subjectdate + '全國業績戰報寄送 ' + subjectnote + '- ' + dateRangeText;
  var body     = '您好，附件為' + subjectdate + '全國業績戰報的 PDF 與 Excel，' + bodynote +
