...
function sent99AnnualBonus() {
  var spreadsheetId    = '1JrR6saVcWD6K0h6J67cgfHz6VDcDRRDMIHseu_f_Nzs';
  var sheetName        = '99年度獎金';
  var controlSheetName = '99年度獎金計算';
  ...
  var subjectdate   = formatDate_99_(controlSheet.getRange('C2').getValue());
  var dateRangeText = formatDateTime_99_(controlSheet.getRange('L2').getValue());
  var sheetTabName  = formatDateCompact_99_(controlSheet.getRange('C2').getValue()) || sheetName;

  var subjectnote = controlSheet.getRange('N2').getValue().toString();
  var bodynote    = controlSheet.getRange('N3').getValue().toString();

  var subject  = subjectdate + '99年度獎金寄送 ' + subjectnote + '- ' + dateRangeText;
  var body     = '您好，附件為' + subjectdate + '99年度獎金的 PDF 與 Excel，' + bodynote +
                 '資料時間為:（' + dateRangeText + '），請查收。';
  var fileName = controlSheet.getRange('C2').getDisplayValue() + '99年度獎金';
  ...
  try {
    ...
    if (!sheet) {
      controlSheet.getRange('L4').setValue('寄送失敗' + getTimestamp_99_());
      SpreadsheetApp.flush();
      return;
    }

    // ---- 篩選資料（第1~3行全要，第4行起 D 欄不為空才納入）----
    var allValues = sheet.getDataRange().getDisplayValues();
    var filteredValues = [];
    var origRowIndices = [];
    for (var i = 0; i < allValues.length; i++) {
      if (i < 3 || allValues[i][3] !== '') {
    ...
    controlSheet.getRange('L4').setValue('寄送成功' + getTimestamp_99_());
    SpreadsheetApp.flush();
    Logger.log('寄送成功' + getTimestamp_99_());
    Logger.log('99年度獎金已寄送，L1 已改為 OFF');

  } catch (e) {
    Logger.log('寄送失敗: ' + e.message);
    controlSheet.getRange('L4').setValue('寄送失敗' + getTimestamp_99_());
    SpreadsheetApp.flush();
    Logger.log('寄送失敗' + getTimestamp_99_());
  }
}

// ===== 工具函式 =====

function formatDate_99_(date) {
  if (!(date instanceof Date)) return String(date);
  var pad = function(n) { return String(n).padStart(2, '0'); };
  return date.getFullYear() + '/' + pad(date.getMonth() + 1) + '/' + pad(date.getDate());
}

function formatDateTime_99_(date) {
  if (!(date instanceof Date)) return String(date);
  var pad = function(n) { return String(n).padStart(2, '0'); };
  return date.getFullYear() + '/' + pad(date.getMonth() + 1) + '/' + pad(date.getDate()) +
...
function formatDateCompact_99_(date) {
  if (!(date instanceof Date)) return String(date);
  var pad = function(n) { return String(n).padStart(2, '0'); };
  return String(date.getFullYear()) + pad(date.getMonth() + 1) + pad(date.getDate());
}

function getTimestamp_99_() {
  var now = new Date();
  var pad = function(n) { return String(n).padStart(2, '0'); };
